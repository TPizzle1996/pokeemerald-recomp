/* Pokémon battle back-sheet / normal-palette / shiny-palette leaves (R9 §7):
 * moved out of this header into the GBA-only TU
 * data/graphics/pokemon_battle_payload.c, so no migrated leaf symbol or byte
 * enters the native object/link graph. This header keeps the still-front
 * leaves (the still table + the external back-EGG row stay compiled on every
 * target) and the unused Deoxys filler block. Do not hand-edit the payload
 * lines: they mirror the family descriptor's source artifacts.
 */
