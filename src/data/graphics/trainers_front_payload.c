/* GBA-only TU: every trainer-front sheet/palette compiled payload (R7B §3).
 *
 * The native target (Makefile_pc, NATIVE_LINUX=1) excludes this TU from its
 * C_SRCS so that no migrated trainer-front leaf symbol or encoded byte enters
 * the native object/link graph; the GBA Makefile auto-collects it via its
 * C_SRCS wildcard, keeping the GBA build's compiled payloads and table
 * pointers exactly as before (R7A ownership: GBA COMPILED).
 *
 * This file is the mechanical extraction of the front-payload lines that used
 * to live in data/graphics/trainers.h (Brendan's pair, split into
 * trainers_brendan_payload.c by R6, is merged back here) in table-index
 * order, TRAINER_PIC_HIKER .. TRAINER_PIC_RS_MAY. Do not hand-edit the
 * payload lines: they mirror the family descriptor's source artifacts.
 */
