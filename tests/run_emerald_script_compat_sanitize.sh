...
# resolver seam (tests/emerald_script_compat_sanitize_test.c).
#
...
echo "== compiling emerald script compat test (ASan/UBSan) =="
cd "$root"
...
gcc "${FLAGS[@]}" \
    ...
    -o "$tmp/emerald_script_compat_sanitize_test"

echo "== running =="
# detect_leaks=0: the harness drives the R6 loader, whose registered
# session (pack + snapshot) is process-lifetime BY DESIGN (the seams'
# no-hot-reload contract; there is no unregister API). The seam's own
# allocations are leak-proven separately: the fault driver's OOM case
# exercises the stage's fail-closed free path, and the teardown order
# shuts every seam down before the leak detector would run.
ASAN_OPTIONS=detect_leaks=0:halt_on_error=1 \
    "$tmp/emerald_script_compat_sanitize_test" \
    "$tmp" \
...
echo "emerald script compat sanitize test passed"
