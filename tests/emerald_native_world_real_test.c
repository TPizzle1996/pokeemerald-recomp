...
int main(int argc, char **argv)
{
    ...
    {
        const char *catalogPaths[32]; /* R15: 21 catalogs now */
    ...
    Gen3ResourcePackDiagnostics_Destroy(&packDiag);
    CHECK(Gen3ResourcePack_GetEntryCount(pack) == 27000u); /* 4518 + 569 audio leaves + 202 structural + 530 song graphs (R12-B/C/D) + 1057 leaf (R13-B) + 5187 text (R13-C) + 3310 D1 + 2081 battle modules (R13-H2) */
    CHECK(Gen3ResourceCatalog_Count(catalog) == 27000u); /* + 569 audio leaves + 202 structural + 530 song graphs (R12-B/C/D) + 1057 leaf (R13-B) + 5187 text (R13-C) + 3310 D1 + 2081 battle modules (R13-H2) */
    ...
    CHECK(info.entryCount == 27000u); /* R15 pack: 27,000 entries */ /* + 569 audio leaves + 202 structural + 530 song graphs (R12-B/C/D) + 1057 leaf (R13-B) + 5187 text (R13-C) + 3310 D1 + 2081 battle modules (R13-H2) */
     ...
     * arena bytes. */
    {
        ...
        CHECK(mergedCount == 5736u); /* 196+2826+288+1544+882 trainer family (R15 pokemon aux) */
        ...
        mergedCount = EmeraldResourceRangeIndex_GetRangeCount(index);
        CHECK(mergedCount == 7037u); /* 5736 + 1301 audio ranges */
     ...
     * via groups_native.inc. Published after the audio seam to mirror the
     * loader chain order (audio first, map second); runs before the
     * neighborhood walks connections/layouts (mapLayout rows come from
     * the layout seam published in step 3). GetPublishedCount is the full
     * publish sum (headers + layouts + event bundles + connections), and
     * the map family registers exactly 3 state-v5 spans (headers array +
     * event arena + connection arena): the shared range index grows
     * 5819 -> 5822 (the loader's 6,377 total is this plus the later
     * script-family 523 + the remaining families' 32). */
    {
        ...
        CHECK(index != NULL);
        CHECK(EmeraldResourceRangeIndex_GetRangeCount(index) == 7040u);
    }
