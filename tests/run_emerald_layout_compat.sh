#!/usr/bin/env bash
#
# R11-D: layout-family compatibility seam tests
# (tests/emerald_layout_compat_test.c).
#
# Compiles the real layout seam against the generated native tables + a
# synthetic snapshot carrying all 882 layout-family resources, then pins
# record publication (pointer fields == arena streams, blockdata/border
# bytes == the patterns), the compiled structural fields surviving
# publication (width/height/tileset refs, incl. UnusedOutdoorArea's NULL
# secondary), transactional failure, republish/clear/shutdown and
# re-init. Pass "sanitize" to build with ASan/UBSan.
set -euo pipefail
...
echo "== compiling layout compat test (real seam + generated tables) =="
cd "$root"
gcc -std=gnu99 -O2 -ffunction-sections -fdata-sections -Wl,--gc-sections \
    ...
    "$here/emerald_layout_compat_test.c" \
    -o "$tmp/emerald_layout_compat_test"
...
if [ "$mode" = "sanitize" ]; then
    ...
    UBSAN_OPTIONS=halt_on_error=1:print_stacktrace=1 \
        "$tmp/emerald_layout_compat_test"
else
    "$tmp/emerald_layout_compat_test"
fi
