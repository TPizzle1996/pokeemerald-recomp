...
def emit_map(map_data, outdir, root, args):
    ...
    # ---- R13-G6: data/maps/groups_native.inc (LINUX64-only) ---------------
    # The native link replaces groups.inc with rows that point at the R13-F
    # seam array gMapHeaders (HOST_DATA, 518 x 48-byte host structs) by
    # routing index instead of the compiled per-map header structs
    # (headers.inc is excluded from the native link). The engine's
    # address-identity path (Overworld_GetMapHeaderByGroupAndId ->
    # gMapGroups -> identity cast -> row) then resolves to the published
    # seam rows (arena events, staged routing mapScripts). The GBA build
    # keeps groups.inc; data/maps.s swaps under the LINUX64 guard.
    group_order = map_data["group_order"]
    gn_lines = [
        "@",
        "@ DO NOT MODIFY THIS FILE! Generated by tools/gen3_resources/",
        "@ gameplay_family/gen_gameplay_family.py (R13-G6 native groups).",
        "@",
        "@ LINUX64-only: the gMapGroup_* rows resolve to the R13-F seam",
        "@ array gMapHeaders (HOST_DATA, 518 x 48-byte rows) by routing",
        "@ index instead of the compiled per-map header structs, so the",
        "@ engine's address-identity map lookup lands on the published",
        "@ seam rows. The GBA build keeps groups.inc.",
        "@",
    ]
    by_group = {}
    for i, h in enumerate(headers):
        by_group.setdefault(h["group"], []).append(i)
    for gi, grp in enumerate(group_order):
        gn_lines.append("")
        gn_lines.append(f"{grp}::")
        for i in by_group[gi]:
            gn_lines.append(f"\t.int gMapHeaders + 48*{i}")
    gn_lines.append("")
    gn_lines.append("\t.align 2")
    gn_lines.append("gMapGroups::")
    for grp in group_order:
        gn_lines.append(f"\t.int {grp}")
    write_if(root / "data/maps/groups_native.inc", gn_lines, args)
