...
bindings for the R1A manifest pipeline:
  ...
  * audio/ownership.generated.toml - [[resources]] per-target ownership
                                     (layout-family grammar): every audio
                                     payload ROM_BASE_ONLY on native
                                     (R12-G removes the compiled objects and
                                     payload includes from the linux64 link)
                                     and COMPILED on GBA
...
def main():
    ...
    # --- emit the audio ownership (R12-G §8) ---
    # Layout-family grammar (resources/extraction/emerald/bpee01/layout/
    # ownership.generated.toml): one [[resources]] record per audio resource,
    # sha256 + sizes per resource, per-target ownership. Every audio payload
    # is ROM_BASE_ONLY on native - R12-G excludes the 530 song objects from
    # the linux64 link and gates the data/sound_data.s payload includes out
    # (voicegroups, keysplits, programmable waves, song table, direct-sound
    # samples) - and COMPILED on GBA. All audio payloads are raw: the
    # encoded bytes ARE the canonical decoded payload, so both hashes are
    # the artifact's sha256 and encoded_length == decoded_length.
    own_lines = [
        "# Generated by tools/gen3_resources/audio_family/gen_audio_bindings.py.",
        "# Do not edit by hand; edit the R12-A inventory and re-run the",
        "# generator (regeneration must be a no-op diff).",
    ...
    for key, kind, symbol, artifact, rtype, schema, rep, size, rom_off, sym_off in rows:
        art_path = root / artifact
        if not art_path.exists():
            fail(f"{key}: ownership artifact {artifact} missing")
        data = art_path.read_bytes()
        if len(data) != size:
            fail(f"{key}: ownership artifact {artifact} is {len(data)} bytes, "
                 f"expected {size}")
        sha = hashlib.sha256(data).hexdigest()
        own_lines.append("[[resources]]")
        own_lines.append(f'id = "{key}"')
        own_lines.append(f'key = "{sha}"')
        own_lines.append(f'legacy_symbol = "{symbol}"')
        own_lines.append(f'type = "{rtype}"')
        own_lines.append(f'source_artifact = "{artifact}"')
        own_lines.append(f"encoded_length = {size}")
        own_lines.append(f"decoded_length = {size}")
        own_lines.append('source_encoding = "raw"')
        own_lines.append(f'source_encoded_sha256 = "{sha}"')
        own_lines.append(f'canonical_decoded_sha256 = "{sha}"')
    ...
    write_if(outdir / "ownership.generated.toml", own_lines)
    ...
    print(f"audio ownership: {len(rows)} records (all ROM_BASE_ONLY native / "
          f"COMPILED GBA, raw encoding)")
