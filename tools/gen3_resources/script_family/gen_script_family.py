...
def render_ownership(modules):
    lines = [
        ...
        "# R13-G6 ownership: every payload is ROM_BASE_ONLY on native - the",
        "# compiled field-script payloads are physically removed from the",
        "# native link; the live generation is staged from the pack (the",
        "# seam registers the 523 ranges and publishes every entrypoint).",
        "# The gba target keeps the qualified ROM bytes as the canonical",
        "# representation.",
        "",
        "ownership_version = 1",
        "game = \"emerald\"",
...
def render_script_native_h(n_mod, n_seg, n_exp, n_rel, n_rrt, n_bnd, n_dyn,
    ...
    pins = [
        ...
        ("EMERALD_SCRIPT_EXPORT_NAME_COUNT", n_exp, 7683),
        ...
        ("EMERALD_SCRIPT_BRAILLE_COUNT", n_braille, 22),
    ]
    lines = [
        "/* Generated by tools/gen3_resources/script_family/gen_script_family.py.",
        " * Do not edit by hand; regeneration must be a no-op diff.",
    ...
    lines += [
        ...
        f"extern uint32_t kBrailleTextAddresses[{n_braille}];",
        f"extern const uint32_t kBrailleGbaAddrs[{n_braille}];",
        "",
...
def render_braille_addresses_inc(rows):
    """R13-G6 (plan sec 7.3): the native-only braille address table.

    `rows` is [(gba, label), ...] sorted ascending by GBA provenance.
    The .inc is included from data/event_scripts.s AFTER braille.inc in
    the same assembly unit, so the LOCAL braille text labels are in
    scope; the GBA build never includes it. The C side pairs
    kBrailleGbaAddrs[i] with kBrailleTextAddresses[i] (declared in
    script_native.generated.h, defined here).
    """
    lines = [
        "/* Generated by tools/gen3_resources/script_family/gen_script_family.py.",
        " * Do not edit by hand; regeneration must be a no-op diff.",
        " *",
        " * R13-G6 (plan sec 7.3): native-only address table for the",
        " * C-owned braille text blocks. Included from data/event_scripts.s",
        " * AFTER braille.inc (same assembly unit, so the LOCAL labels are",
        " * in scope); rows are ordered by ascending GBA provenance and",
        " * pair with kBrailleGbaAddrs in script_native_table.generated.c.",
        " * The GBA build does not include this file - the labels are",
        " * direct there. */",
        "\t.globl kBrailleTextAddresses",
        "kBrailleTextAddresses:",
    ]
    for gba, label in rows:
        lines.append(f"\t.int {label} /* 0x{gba:x} */")
    return "\n".join(lines) + "\n"
...
def render_script_native_c(mod_rows, seg_rows, exp_rows, reloc_rows, rrt_rows,
    ...
    out = [
        ...
        "",
        "/* The 3 recomp-local movement bridges (plan sec 5/7.4): B-owned",
        " * movement bytes, 12 B total. The table embeds the qualified-ROM",
        " * bytes directly; the seam's SCRIPT_BRIDGE/MOVEMENT_BRIDGE",
        " * resolution returns those embedded bytes as the live pointer",
        " * (R13-G6: no compiled-symbol extern remains - the movement",
        " * isolation exception stays named, never linked). */",
        ...
        "",
        "/* R13-G6 (plan sec 7.3): braille text GBA provenance, sorted",
        " * ascending - the seam bsearches this by encoded GBA target.",
        " * Index i pairs with kBrailleTextAddresses[i] (defined in",
        " * data/text/braille_addresses_native.inc, the same assembly unit",
        " * as braille.inc), so the 26 BRAILLE reloc rows resolve live",
        " * through the compiled C-owned braille text. */",
        f"const uint32_t kBrailleGbaAddrs[{len(braille_gba_rows)}] = {{",
    ]
    for gba, _label in braille_gba_rows:
        out.append(f"    0x{gba:x}u,")
    out += [
        "};",
        "",
    ]
    ...
    emit_array("kScriptExports", "struct EmeraldScriptNativeExport",
               ...
               lambda r: "{%uu, %uu, 0x%xu, %uu, %uu, %uu}" % (
                   ...
                   r["name"]))
    # R13-G6: C-site export name index, sorted by name for the seam's
    # bsearch (EmeraldScriptCompat_GetScriptSymbol). Every engine C
    # reference to a G entrypoint resolves through this index; the 3
    # movement bridges resolve via kScriptBridges instead. Names are
    # unique (each row names exactly one export).
    name_rows = []
    for i, e in enumerate(exp_rows):
        nm = pool[e["name"]] if e["name"] != 0xFFFFFFFF else None
        if nm:
            name_rows.append((nm, i))
    name_rows.sort(key=lambda r: r[0])
    if len(name_rows) != len(exp_rows):
        fail(f"export name index covers {len(name_rows)}/{len(exp_rows)} "
             f"exports (every export must carry a name)")
    for i in range(1, len(name_rows)):
        if name_rows[i - 1][0] == name_rows[i][0]:
            fail(f"duplicate export name {name_rows[i][0]} "
                 f"(rows {name_rows[i-1][1]}/{name_rows[i][1]})")
    emit_array("kScriptExportNameIndex",
               "struct EmeraldScriptNativeExportName", name_rows,
               lambda r: "{%s, %uu}" % (_cstr(r[0]), r[1]))
    ...
    out += [
        ...
        "    kScriptExportNameIndex, EMERALD_SCRIPT_EXPORT_NAME_COUNT,",
...
def emit_script_native_tables(root, args, rom, base, modules, routing_relocs,
    ...
    # R13-G6 (plan sec 7.3) braille C handoff: the BRAILLE-kind reloc
    # rows (26 edges) resolve live through a generated address table
    # instead of deferring. Dedupe by GBA provenance (some addresses
    # are referenced by more than one operand) into the 22 label rows;
    # the label comes from the reloc row's target_export (braille rows
    # have no canonical_label - no C catalog record exists yet).
    braille_gba = {}
    for r in reloc_rows:
        if r["kind"] != "BRAILLE":
            continue
        lbl = pool[r["label"]]
        prev = braille_gba.get(r["enc"])
        if prev is not None and prev != lbl:
            fail(f"braille address 0x{r['enc']:x} has two labels: "
                 f"{prev} / {lbl}")
        braille_gba[r["enc"]] = lbl
    if len(braille_gba) != BRAILLE_TEXT_LABELS_PIN:
        fail(f"braille address rows {len(braille_gba)} "
             f"!= {BRAILLE_TEXT_LABELS_PIN}")
    braille_gba_rows = sorted(braille_gba.items())
    ...
    body = render_script_native_c(
        ...
        bound_rows, rseg_rows, pool, arena_bytes, braille_gba_rows)
    ...
    # R13-G6 (plan sec 7.3): the braille address table is asm (it names
    # the LOCAL braille.inc labels); included from data/event_scripts.s
    # inside the same native-only guard as the carved text blocks. The
    # GBA build never sees it - the labels are direct there.
    braille_inc_path = root / "data/text/braille_addresses_native.inc"
    write_if(braille_inc_path,
             render_braille_addresses_inc(braille_gba_rows), args.check)
    print(f"    braille handoff: {len(braille_gba_rows)} address rows -> "
          f"{braille_inc_path.name}")
